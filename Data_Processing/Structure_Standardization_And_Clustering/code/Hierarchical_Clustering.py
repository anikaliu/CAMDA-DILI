...
for i,row in df_iter.iterrows():
    if row[0] <= 922:
        dict_dend[i+923] = [row[0]]
        
    if row[0] > 922:
        dict_dend[i+923] = [j for j in dict_dend[row[0]]]
        
              
    if row[1] <= 922:
        dict_dend[i+923].append(row[1])
        
    if row[1] > 922:
        for j in dict_dend[row[1]]:
            dict_dend[i+923].append(j)
            

#which clusters are present at distance cut-off 0.5?

target_set = set(range(923))
...
clusters = [dict_cluster[i] for i in range(923)]
