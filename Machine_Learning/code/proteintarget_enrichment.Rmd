...
```
```{r}
df_plotting$wilcox=wilcox
df_sig<-df_plotting%>%filter(wilcox<0.05)

df_sig$ecdf_SVMmcnc=(ecdf(df_plotting$SVM_mcnc))
df_threshold=df_plotting%>%filter(`Enrichment in DILI compounds`=='Significant')%>%dplyr::select(SVM_mcnc,RF_mcnc)%>%
  pivot_longer(names_to = 'model', values_to='importance',cols=c('SVM_mcnc','RF_mcnc'))


ggplot(df_threshold, aes(importance, color=model))+
  stat_ecdf()+
...
#RF_mcnc
#for (index in 2:7){
for (name in c('RF_mcnc','SVM_mcnc')){
#  name<-colnames(df_featimp)[index]
  ...
  ggsave(pheatmap, filename=paste0('../plots/enrichment_',name,'.pdf'), height = 20, width = 20)
  }
}
```
...
SI_table<-allfeat%>%filter(Uniprot %in%df_sig$Uniprot)%>%inner_join(df_featimp)%>%left_join(df_targetinfo)%>%
  filter(RF_mcnc>quantile(df_sig[,which(colnames(df_sig)=='RF_mcnc')], probs=0.97)|
           SVM_mcnc>quantile(df_sig[,which(colnames(df_sig)=='SVM_mcnc')], probs=0.97))%>%filter(mcnc<0.05)%>%dplyr::select(Symbol,Name, Protein_Classification, RF_mcnc, SVM_mcnc)
