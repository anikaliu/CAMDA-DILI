...
df_standardmedians<-df_performance%>%
  filter(true_or_y_scrambled_labels_used ==1)%>%
  group_by(condition, dataset, testset, dataset_readable)%>%summarise("median_balacc"=median(balanced_accuracy, na.rm=T))%>%mutate('mean'="Mean performance")
